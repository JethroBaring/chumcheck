<script lang="ts">
	...
	const currentModule = $derived($page.url.pathname.slice(1));
</script>

<header
	class="flex h-16 w-screen justify-center border-b text-flutter-gray backdrop-blur-lg dark:text-flutter-white"
>
	<nav class="flex h-16 w-4/5 items-center px-0">
		<div class="flex flex-1 cursor-pointer gap-2">
			...
			<a href={`/${modules[0].module.toLowerCase()}${modules[0].subModule.length > 0 ? '/' + modules[0].subModule[0].toLowerCase() : ''}`} class="cursor-pointer text-xl font-black normal-case">ChumCheck</a>
		</div>
			...
			<DropdownMenu.Root>
				...
				<DropdownMenu.Content align="end">
					<DropdownMenu.Group>
						...
						{#each modules as module}
							<a href={`/${module.module.toLowerCase()}${module.subModule.length > 0 ? '/' + module.subModule[0].toLowerCase() : ''}`}>

<script lang="ts">
	...
	async function searchUsers() {
		const response = await fetch(`${PUBLIC_API_URL}/users/?search=${search}`, {
			method: 'GET',
			headers: {
				Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNzI5MDk5NjcwLCJpYXQiOjE3Mjg4MTc2MjQsImp0aSI6IjIyNmIxYTQ0ZTQ4YzQzMmZhNDJjOGVhZDNmMmJjNjQzIiwidXNlcl9pZCI6MSwidXNlcl90eXBlIjoiTSJ9.SGiRwgg-yrwdShhwaFHxXsnypcUAd7Na-onQLH36CVY`
			}
		});
...
<div class="flex-1 overflow-scroll px-1" class:hidden={currentActive !== 3}>
		...
		<div class="grid gap-2">
			...
			<input type="hidden" name={`member_${i+2}`} value={user.id}>
				<div class="flex items-center justify-between gap-3">
					...
					<button on:click|preventDefault={() => removeMember(user)}> <Add class="h-4 w-4" /> </button>
				</div>
			{/each}
		</div>

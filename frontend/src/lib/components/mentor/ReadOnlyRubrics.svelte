<script lang="ts">
	...
	export let questions: any, type: string, scores: any, readiness_level: any
	console.log(type);
</script>
	...
	<div class="flex flex-col gap-3">
		...
		<div class="flex gap-3">
			<input
				...
				class="radio"
				value={question.id}
				checked={readiness_level.readiness_level === index+1}
				disabled
			/>
					...
					<Table.Body>
						{#each question.level_criteria as criteria, i}
							<Table.Row class="h-[50px]">
								<input type="hidden" name={`${type}Criteria${question.level}${i+1}`} value={`${criteria.id}`}>
								...
								<Table.Cell class="text-center"
									><input
										...
										value="5"
										disabled
										checked={scores[(index)*6+i].score === 5}
									/></Table.Cell
								>
								<Table.Cell class="text-center"
									><input
										...
										disabled
										checked={scores[(index)*6+i].score === 4}
									/></Table.Cell
								>
								<Table.Cell class="text-center"
									><input
										...
										disabled
										checked={scores[(index)*6+i].score === 3}
									/></Table.Cell
								>
								<Table.Cell class="text-center"
									><input
										...
										disabled
										checked={scores[(index)*6+i].score === 2}
									/></Table.Cell
								>
								<Table.Cell class="text-center"
									><input
										...
										disabled
										checked={scores[(index)*6+i].score === 1}
									/></Table.Cell
								>
								<Table.Cell class="text-center">
									<textarea

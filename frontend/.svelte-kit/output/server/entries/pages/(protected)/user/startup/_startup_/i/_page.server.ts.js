...
const load = async ({ fetch, cookies, params }) => {
	const response = await fetch(`${PUBLIC_API_URL}/startups/${params.startup}/allow-initiatives/`, {
		method: 'get',
		headers: {
			...
			try {
				const initiativesResults = await Promise.all(
					tasks_ids.map(async (task) => {
						const response2 = await fetch(`${PUBLIC_API_URL}/tasks/initiatives/?task_id=${task}`, {
							method: 'get',
							headers: {
								Authorization: `Bearer ${cookies.get('Access')}`
							}
